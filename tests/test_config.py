...
def test_save_and_load_config_roundtrip(tmp_path):
    ...
    with patch("bot.config.BOT_DIR", bot_dir), \
        ...
        from bot.config import save_config, load_config
...
def test_save_history_and_load_history(tmp_path):
    ...
    bot_dir.mkdir()
    with patch("bot.config.BOT_DIR", bot_dir):
        from bot.config import save_history, load_history
        messages = [
...
def test_clear_history_removes_file(tmp_path):
    ...
    bot_dir.mkdir()
    with patch("bot.config.BOT_DIR", bot_dir):
        from bot.config import save_history, clear_history, load_history
...
def test_history_trimmed_to_max(tmp_path):
    ...
    bot_dir.mkdir()
    with patch("bot.config.BOT_DIR", bot_dir), \
         patch("bot.config.MAX_HISTORY", 2):
        from bot.config import save_history, load_history
