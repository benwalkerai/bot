...
- [ ] **Token/cost tracking** — per-message and cumulative totals, broken down by provider
- [ ] **Session listing improvements** — show provider used and first message as a preview
- [ ] **Session export** — dump a session to markdown or plain text
- [ ] **Interactive mode** — `bot --chat` for a persistent REPL instead of one-shot messages
- [ ] **System prompt override** — `--system` flag to replace the default prompt per query

## Developer Experience

- [ ] **CI on every push** — run tests and ruff check on every PR and push to main, not just tags
